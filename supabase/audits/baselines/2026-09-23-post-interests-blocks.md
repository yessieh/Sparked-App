...
indication that it has done so. Sections 1 and 5 both exceed that and were
paged and concatenated into one continuous block each:

| Section        | Rows | Pages implied by the count | count(*) companion |
| -------------- | ---- | -------------------------- | ------------------ |
| 1 — Grants     | 118  | 2 (100 + 18)               | 1A = 118           |
| 5 — Privileges | 268  | 3 (100 + 100 + 68)         | 5A = 268           |

**Both totals EQUAL their count(*) companions**, run separately in the SQL
Editor and recorded here. A truncated section 1 would have landed on 100; a
truncated section 5 on 100 or 200. Neither did.

## QA-0034 RESULT

qa-0034 run 2026-09-22 by Jas in SQL Editor: Section 1 12/12, Section 2 30/30,
rollback confirmed. Settles the migration header's 'UNVERIFIED until that run
is recorded' notes (0034 is applied and immutable, so they are settled here).
...
| public | category_preferences  | column      | stance             | authenticated | UPDATE         |
| public | category_preferences  | table       | (table-level)      | authenticated | DELETE         |
| public | category_preferences  | table       | (table-level)      | authenticated | INSERT         |
| public | category_preferences  | table       | (table-level)      | authenticated | SELECT         |
...
| public              | category_preferences  | true        | false      | 4            |
...
| app    | events_within_radius     | p_origin_lat double precision, p_origin_lng double precision, p_radius_miles double precision, p_from timestamp with time zone, p_to timestamp with time zone     | true             | postgres | search_path=public, app, extensions | postgres:EXECUTE, anon:EXECUTE, authenticated:EXECUTE                 |
...
| app    | is_event_member          | p_event_id uuid, p_roles text[]                                                                                                                                   | true             | postgres | search_path=public, app             | postgres:EXECUTE, anon:EXECUTE, authenticated:EXECUTE                 |
...
| app    | curbside_expired         | p_tier_id text, p_starts_at timestamp with time zone, p_ends_at timestamp with time zone                                                                          | false            | postgres | search_path=public, app             | postgres:EXECUTE, anon:EXECUTE, authenticated:EXECUTE                 |
...
| public | events_within_radius     | origin_lat double precision, origin_lng double precision, radius_miles double precision, window_from timestamp with time zone, window_to timestamp with time zone | false            | postgres | search_path=public, app             | postgres:EXECUTE, anon:EXECUTE, authenticated:EXECUTE                 |
...
| schema | table_name            | policy_name                      | command | roles         | using_expr                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | using_md5                        | with_check_expr                                                                                                                                       | with_check_md5                   |
| ------ | --------------------- | -------------------------------- | ------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| public | categories            | categories_select_public         | SELECT  | PUBLIC        | true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | b326b5062b2f0e69046810717534cb09 | null                                                                                                                                                  | null                             |
| public | category_preferences  | category_preferences_delete_own  | DELETE  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | category_preferences  | category_preferences_insert_own  | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | (user_id = auth.uid())                                                                                                                                | 3a807491e0a912fbfbee0ffa087b82f6 |
| public | category_preferences  | category_preferences_select_own  | SELECT  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | category_preferences  | category_preferences_update_own  | UPDATE  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | (user_id = auth.uid())                                                                                                                                | 3a807491e0a912fbfbee0ffa087b82f6 |
| public | curbside_quota_ledger | curbside_quota_ledger_select_own | SELECT  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | event_categories      | event_categories_delete_members  | DELETE  | authenticated | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_categories.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text]))))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 8d73d16e89c37ff628e025e9293fae2f | null                                                                                                                                                  | null                             |
| public | event_categories      | event_categories_insert_members  | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_categories.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text])))) | 8d73d16e89c37ff628e025e9293fae2f |
| public | event_categories      | event_categories_select_public   | SELECT  | PUBLIC        | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_categories.event_id) AND (((e.deleted_at IS NULL) AND app.is_event_member(e.id, ARRAY['owner'::text, 'editor'::text, 'viewer'::text])) OR ((e.deleted_at IS NULL) AND (e.archived_at IS NULL) AND (e.status = ANY (ARRAY['published'::text, 'cancelled'::text])) AND (NOT app.curbside_expired(e.tier_id, e.starts_at, e.ends_at))) OR ((e.status = ANY (ARRAY['published'::text, 'cancelled'::text])) AND (COALESCE(e.ends_at, (e.starts_at + '03:00:00'::interval)) < now()) AND (NOT app.curbside_expired(e.tier_id, e.starts_at, e.ends_at)) AND app.has_attendance(e.id)))))) | a51a4139880b6e2e4daf29b00f01390b | null                                                                                                                                                  | null                             |
| public | event_categories      | event_categories_update_members  | UPDATE  | authenticated | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_categories.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text]))))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 8d73d16e89c37ff628e025e9293fae2f | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_categories.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text])))) | 8d73d16e89c37ff628e025e9293fae2f |
| public | event_vendors         | event_vendors_delete_members     | DELETE  | authenticated | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_vendors.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text]))))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 41c8ed7d9234592ca863c5f259c48ae1 | null                                                                                                                                                  | null                             |
| public | event_vendors         | event_vendors_insert_members     | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_vendors.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text]))))    | 41c8ed7d9234592ca863c5f259c48ae1 |
| public | event_vendors         | event_vendors_select_public      | SELECT  | PUBLIC        | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_vendors.event_id) AND (((e.status = ANY (ARRAY['published'::text, 'cancelled'::text])) AND (e.archived_at IS NULL)) OR app.is_event_member(e.id, ARRAY['owner'::text, 'editor'::text, 'viewer'::text])))))                                                                                                                                                                                                                                                                                                                                                                         | 0c5031e11eca14b1c898b14181ef444d | null                                                                                                                                                  | null                             |
| public | event_vendors         | event_vendors_update_members     | UPDATE  | authenticated | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_vendors.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text]))))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 41c8ed7d9234592ca863c5f259c48ae1 | (EXISTS ( SELECT 1 FROM events e WHERE ((e.id = event_vendors.event_id) AND app.is_member(e.workspace_id, ARRAY['owner'::text, 'editor'::text]))))    | 41c8ed7d9234592ca863c5f259c48ae1 |
| public | events                | events_delete_owner              | DELETE  | authenticated | app.is_member(workspace_id, ARRAY['owner'::text])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 928b21ac9c83be48ba1e516ad038ba43 | null                                                                                                                                                  | null                             |
| public | events                | events_insert_members            | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | app.is_member(workspace_id, ARRAY['owner'::text, 'editor'::text])                                                                                     | 2e35efc3d160f18b3710478423a681bd |
| public | events                | events_select_public             | SELECT  | PUBLIC        | (((deleted_at IS NULL) AND app.is_member(workspace_id, ARRAY['owner'::text, 'editor'::text, 'viewer'::text])) OR ((deleted_at IS NULL) AND (archived_at IS NULL) AND (status = ANY (ARRAY['published'::text, 'cancelled'::text])) AND (NOT app.curbside_expired(tier_id, starts_at, ends_at))) OR ((status = ANY (ARRAY['published'::text, 'cancelled'::text])) AND (COALESCE(ends_at, (starts_at + '03:00:00'::interval)) < now()) AND (NOT app.curbside_expired(tier_id, starts_at, ends_at)) AND app.has_attendance(id)))                                                                                                             | bd2a51ba81b5de7d6eb288bcdeeb340d | null                                                                                                                                                  | null                             |
| public | events                | events_update_members            | UPDATE  | authenticated | app.is_member(workspace_id, ARRAY['owner'::text, 'editor'::text])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 2e35efc3d160f18b3710478423a681bd | app.is_member(workspace_id, ARRAY['owner'::text, 'editor'::text])                                                                                     | 2e35efc3d160f18b3710478423a681bd |
| public | memberships           | memberships_select_own           | SELECT  | PUBLIC        | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | profiles              | profiles_select_own              | SELECT  | PUBLIC        | (id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ea1f5df45681e1946eaa85c22966d773 | null                                                                                                                                                  | null                             |
| public | profiles              | profiles_update_own              | UPDATE  | PUBLIC        | (id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ea1f5df45681e1946eaa85c22966d773 | (id = auth.uid())                                                                                                                                     | ea1f5df45681e1946eaa85c22966d773 |
| public | rsvps                 | rsvps_delete_own                 | DELETE  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | rsvps                 | rsvps_insert_own                 | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | (user_id = auth.uid())                                                                                                                                | 3a807491e0a912fbfbee0ffa087b82f6 |
| public | rsvps                 | rsvps_select_own                 | SELECT  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | saves                 | saves_delete_own                 | DELETE  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | saves                 | saves_insert_own                 | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | (user_id = auth.uid())                                                                                                                                | 3a807491e0a912fbfbee0ffa087b82f6 |
| public | saves                 | saves_select_own                 | SELECT  | authenticated | (user_id = auth.uid())                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 3a807491e0a912fbfbee0ffa087b82f6 | null                                                                                                                                                  | null                             |
| public | tier_prices           | tier_prices_select_public        | SELECT  | PUBLIC        | true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | b326b5062b2f0e69046810717534cb09 | null                                                                                                                                                  | null                             |
| public | tiers                 | tiers_select_public              | SELECT  | PUBLIC        | true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | b326b5062b2f0e69046810717534cb09 | null                                                                                                                                                  | null                             |
| public | workspaces            | workspaces_delete_owner          | DELETE  | authenticated | app.is_member(id, ARRAY['owner'::text])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 2847ba21290ebebf479ed5190c23296e | null                                                                                                                                                  | null                             |
| public | workspaces            | workspaces_insert_auth           | INSERT  | authenticated | null                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | null                             | (created_by = auth.uid())                                                                                                                             | 987767fc803474751ce44a4235a12922 |
| public | workspaces            | workspaces_select_public         | SELECT  | PUBLIC        | true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | b326b5062b2f0e69046810717534cb09 | null                                                                                                                                                  | null                             |
| public | workspaces            | workspaces_update_owner          | UPDATE  | authenticated | app.is_member(id, ARRAY['owner'::text])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 2847ba21290ebebf479ed5190c23296e | app.is_member(id, ARRAY['owner'::text])                                                                                                               | 2847ba21290ebebf479ed5190c23296e |
